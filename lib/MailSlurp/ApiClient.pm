...
sub new {
    ...
    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'MailSlurp::Configuration' ) {
        $config = $_[0];
    } else {
        $config = MailSlurp::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("MailSlurp::Object::$class")->new;
        if (ref $data eq "HASH") {
