...
# @param string $contact_id  (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'get_contact_v_card' } = { 
...
# @return ARRAY[string]
#
sub get_contact_v_card {
    my ($self, %args) = @_;

    # verify the required parameter 'contact_id' is set
    unless (exists $args{'contact_id'}) {
      croak("Missing the required parameter 'contact_id' when calling get_contact_v_card");
    }

    # parse inputs
    my $_resource_path = '/contacts/{contactId}/download';
