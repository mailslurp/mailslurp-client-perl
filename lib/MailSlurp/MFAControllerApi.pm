...
# @param CreateTotpDeviceBase32SecretKeyOptions $create_totp_device_base32_secret_key_options  (required)
{
    ...
    'create_totp_device_base32_secret_key_options' => {
        data_type => 'CreateTotpDeviceBase32SecretKeyOptions',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'create_totp_device_for_base32_secret_key' } = { 
        summary => 'Create a TOTP device from an base32 secret key',
        params => $params,
...
#
sub create_totp_device_for_base32_secret_key {
    my ($self, %args) = @_;

    # verify the required parameter 'create_totp_device_base32_secret_key_options' is set
    unless (exists $args{'create_totp_device_base32_secret_key_options'}) {
      croak("Missing the required parameter 'create_totp_device_base32_secret_key_options' when calling create_totp_device_for_base32_secret_key");
    }

    # parse inputs
    my $_resource_path = '/mfa/totp/device/base32SecretKey';
    ...
    my $_body_data;
    # body params
    if ( exists $args{'create_totp_device_base32_secret_key_options'}) {
        $_body_data = $args{'create_totp_device_base32_secret_key_options'};
    }
...
# @param CreateTotpDeviceCustomOptions $create_totp_device_custom_options  (required)
{
    ...
    'create_totp_device_custom_options' => {
        data_type => 'CreateTotpDeviceCustomOptions',
        description => '',
        required => '1',
    },
    };
...
sub create_totp_device_for_custom {
    ...
    unless (exists $args{'create_totp_device_custom_options'}) {
      croak("Missing the required parameter 'create_totp_device_custom_options' when calling create_totp_device_for_custom");
    }
    ...
    my $_body_data;
    # body params
    if ( exists $args{'create_totp_device_custom_options'}) {
        $_body_data = $args{'create_totp_device_custom_options'};
    }
...
# @param int $min_seconds_until_expire Optional minimum time until code expires. Will hold thread open until period reached. (optional, default to 5)
{
    ...
    'id' => {
        data_type => 'string',
        description => 'ID of the TOTP device to get the code for',
        required => '1',
    },
    'at' => {
        data_type => 'DateTime',
        description => 'Optional time to get code for. If not provided, current time is used.',
        required => '0',
    },
    'min_seconds_until_expire' => {
        data_type => 'int',
        description => 'Optional minimum time until code expires. Will hold thread open until period reached.',
        required => '0',
    },
    };
...
sub get_totp_device_code {
    ...
    # query params
    if ( exists $args{'min_seconds_until_expire'}) {
        $query_params->{'minSecondsUntilExpire'} = $self->{api_client}->to_query_value($args{'min_seconds_until_expire'});
    }
