...
sub create_webhook {
    ...
    unless (exists $args{'webhook_options'}) {
      croak("Missing the required parameter 'webhook_options' when calling create_webhook");
    }
    ...
    my $_body_data;
    # body params
    if ( exists $args{'webhook_options'}) {
        $_body_data = $args{'webhook_options'};
    }
...
sub get_webhook_result {
    ...
    my $_response_object = $self->{api_client}->deserialize('WebhookResultEntity', $response);
    return $_response_object;
}
