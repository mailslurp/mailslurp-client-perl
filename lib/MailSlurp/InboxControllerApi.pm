=begin comment

MailSlurp API

MailSlurp is an API for sending and receiving emails and SMS from dynamically allocated email addresses and phone numbers. It's designed for developers and QA teams to test applications, process inbound emails, send templated notifications, attachments, and more.  ## Resources  - [Homepage](https://www.mailslurp.com) - Get an [API KEY](https://app.mailslurp.com/sign-up/) - Generated [SDK Clients](https://docs.mailslurp.com/) - [Examples](https://github.com/mailslurp/examples) repository

The version of the OpenAPI document: 6.5.2
Contact: contact@mailslurp.dev
Generated by: https://openapi-generator.tech

=end comment

=cut

#
# NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
# Do not edit the class manually.
# Ref: https://openapi-generator.tech
#
package MailSlurp::InboxControllerApi;

require 5.6.0;
use strict;
use warnings;
use utf8; 
use Exporter;
use Carp qw( croak );
use Log::Any qw($log);

use MailSlurp::ApiClient;

use base "Class::Data::Inheritable";

__PACKAGE__->mk_classdata('method_documentation' => {});

sub new {
    my $class = shift;
    my $api_client;

    if ($_[0] && ref $_[0] && ref $_[0] eq 'MailSlurp::ApiClient' ) {
        $api_client = $_[0];
    } else {
        $api_client = MailSlurp::ApiClient->new(@_);
    }

    bless { api_client => $api_client }, $class;

}


#
# cancel_scheduled_job
#
# Cancel a scheduled email job
# 
# @param string $job_id  (required)
{
    my $params = {
    'job_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'cancel_scheduled_job' } = { 
        summary => 'Cancel a scheduled email job',
        params => $params,
        returns => 'ScheduledJobDto',
        };
}
# @return ScheduledJobDto
#
sub cancel_scheduled_job {
    my ($self, %args) = @_;

    # verify the required parameter 'job_id' is set
    unless (exists $args{'job_id'}) {
      croak("Missing the required parameter 'job_id' when calling cancel_scheduled_job");
    }

    # parse inputs
    my $_resource_path = '/inboxes/scheduled-jobs/{jobId}';

    my $_method = 'DELETE';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # path params
    if ( exists $args{'job_id'}) {
        my $_base_variable = "{" . "jobId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'job_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('ScheduledJobDto', $response);
    return $_response_object;
}

#
# create_inbox
#
# Create an inbox email address. An inbox has a real email address and can send and receive emails. Inboxes can be either `SMTP` or `HTTP` inboxes.
# 
# @param string $email_address A custom email address to use with the inbox. Defaults to null. When null MailSlurp will assign a random email address to the inbox such as &#x60;123@mailslurp.com&#x60;. If you use the &#x60;useDomainPool&#x60; option when the email address is null it will generate an email address with a more varied domain ending such as &#x60;123@mailslurp.info&#x60; or &#x60;123@mailslurp.biz&#x60;. When a custom email address is provided the address is split into a domain and the domain is queried against your user. If you have created the domain in the MailSlurp dashboard and verified it you can use any email address that ends with the domain. Note domain types must match the inbox type - so &#x60;SMTP&#x60; inboxes will only work with &#x60;SMTP&#x60; type domains. Avoid &#x60;SMTP&#x60; inboxes if you need to send emails as they can only receive. Send an email to this address and the inbox will receive and store it for you. To retrieve the email use the Inbox and Email Controller endpoints with the inbox ID. (optional)
# @param ARRAY[string] $tags Tags that inbox has been tagged with. Tags can be added to inboxes to group different inboxes within an account. You can also search for inboxes by tag in the dashboard UI. (optional)
# @param string $name Optional name of the inbox. Displayed in the dashboard for easier search and used as the sender name when sending emails. (optional)
# @param string $description Optional description of the inbox for labelling purposes. Is shown in the dashboard and can be used with (optional)
# @param boolean $use_domain_pool Use the MailSlurp domain name pool with this inbox when creating the email address. Defaults to null. If enabled the inbox will be an email address with a domain randomly chosen from a list of the MailSlurp domains. This is useful when the default &#x60;@mailslurp.com&#x60; email addresses used with inboxes are blocked or considered spam by a provider or receiving service. When domain pool is enabled an email address will be generated ending in &#x60;@mailslurp.{world,info,xyz,...}&#x60; . This means a TLD is randomly selecting from a list of &#x60;.biz&#x60;, &#x60;.info&#x60;, &#x60;.xyz&#x60; etc to add variance to the generated email addresses. When null or false MailSlurp uses the default behavior of &#x60;@mailslurp.com&#x60; or custom email address provided by the emailAddress field. Note this feature is only available for &#x60;HTTP&#x60; inbox types. (optional)
# @param boolean $favourite Is the inbox a favorite. Marking an inbox as a favorite is typically done in the dashboard for quick access or filtering (optional)
# @param DateTime $expires_at Optional inbox expiration date. If null then this inbox is permanent and the emails in it won&#39;t be deleted. If an expiration date is provided or is required by your plan the inbox will be closed when the expiration time is reached. Expired inboxes still contain their emails but can no longer send or receive emails. An ExpiredInboxRecord is created when an inbox and the email address and inbox ID are recorded. The expiresAt property is a timestamp string in ISO DateTime Format yyyy-MM-dd&#39;T&#39;HH:mm:ss.SSSXXX. (optional)
# @param int $expires_in Number of milliseconds that inbox should exist for (optional)
# @param boolean $allow_team_access DEPRECATED (team access is always true). Grant team access to this inbox and the emails that belong to it for team members of your organization. (optional)
# @param string $inbox_type HTTP (default) or SMTP inbox type. HTTP inboxes are default and best solution for most cases. SMTP inboxes are more reliable for public inbound email consumption (but do not support sending emails). When using custom domains the domain type must match the inbox type. HTTP inboxes are processed by AWS SES while SMTP inboxes use a custom mail server running at &#x60;mxslurp.click&#x60;. (optional)
# @param boolean $virtual_inbox Virtual inbox prevents any outbound emails from being sent. It creates sent email records but will never send real emails to recipients. Great for testing and faking email sending. (optional)
# @param boolean $use_short_address Use a shorter email address under 31 characters (optional)
# @param string $domain_id ID of custom domain to use for email address. (optional)
# @param string $domain_name FQDN domain name for the domain you have verified. Will be appended with a randomly assigned recipient name. Use the &#x60;emailAddress&#x60; option instead to specify the full custom inbox. (optional)
# @param string $prefix Prefix to add before the email address for easier labelling or identification. (optional)
{
    my $params = {
    'email_address' => {
        data_type => 'string',
        description => 'A custom email address to use with the inbox. Defaults to null. When null MailSlurp will assign a random email address to the inbox such as &#x60;123@mailslurp.com&#x60;. If you use the &#x60;useDomainPool&#x60; option when the email address is null it will generate an email address with a more varied domain ending such as &#x60;123@mailslurp.info&#x60; or &#x60;123@mailslurp.biz&#x60;. When a custom email address is provided the address is split into a domain and the domain is queried against your user. If you have created the domain in the MailSlurp dashboard and verified it you can use any email address that ends with the domain. Note domain types must match the inbox type - so &#x60;SMTP&#x60; inboxes will only work with &#x60;SMTP&#x60; type domains. Avoid &#x60;SMTP&#x60; inboxes if you need to send emails as they can only receive. Send an email to this address and the inbox will receive and store it for you. To retrieve the email use the Inbox and Email Controller endpoints with the inbox ID.',
        required => '0',
    },
    'tags' => {
        data_type => 'ARRAY[string]',
        description => 'Tags that inbox has been tagged with. Tags can be added to inboxes to group different inboxes within an account. You can also search for inboxes by tag in the dashboard UI.',
        required => '0',
    },
    'name' => {
        data_type => 'string',
        description => 'Optional name of the inbox. Displayed in the dashboard for easier search and used as the sender name when sending emails.',
        required => '0',
    },
    'description' => {
        data_type => 'string',
        description => 'Optional description of the inbox for labelling purposes. Is shown in the dashboard and can be used with',
        required => '0',
    },
    'use_domain_pool' => {
        data_type => 'boolean',
        description => 'Use the MailSlurp domain name pool with this inbox when creating the email address. Defaults to null. If enabled the inbox will be an email address with a domain randomly chosen from a list of the MailSlurp domains. This is useful when the default &#x60;@mailslurp.com&#x60; email addresses used with inboxes are blocked or considered spam by a provider or receiving service. When domain pool is enabled an email address will be generated ending in &#x60;@mailslurp.{world,info,xyz,...}&#x60; . This means a TLD is randomly selecting from a list of &#x60;.biz&#x60;, &#x60;.info&#x60;, &#x60;.xyz&#x60; etc to add variance to the generated email addresses. When null or false MailSlurp uses the default behavior of &#x60;@mailslurp.com&#x60; or custom email address provided by the emailAddress field. Note this feature is only available for &#x60;HTTP&#x60; inbox types.',
        required => '0',
    },
    'favourite' => {
        data_type => 'boolean',
        description => 'Is the inbox a favorite. Marking an inbox as a favorite is typically done in the dashboard for quick access or filtering',
        required => '0',
    },
    'expires_at' => {
        data_type => 'DateTime',
        description => 'Optional inbox expiration date. If null then this inbox is permanent and the emails in it won&#39;t be deleted. If an expiration date is provided or is required by your plan the inbox will be closed when the expiration time is reached. Expired inboxes still contain their emails but can no longer send or receive emails. An ExpiredInboxRecord is created when an inbox and the email address and inbox ID are recorded. The expiresAt property is a timestamp string in ISO DateTime Format yyyy-MM-dd&#39;T&#39;HH:mm:ss.SSSXXX.',
        required => '0',
    },
    'expires_in' => {
        data_type => 'int',
        description => 'Number of milliseconds that inbox should exist for',
        required => '0',
    },
    'allow_team_access' => {
        data_type => 'boolean',
        description => 'DEPRECATED (team access is always true). Grant team access to this inbox and the emails that belong to it for team members of your organization.',
        required => '0',
    },
    'inbox_type' => {
        data_type => 'string',
        description => 'HTTP (default) or SMTP inbox type. HTTP inboxes are default and best solution for most cases. SMTP inboxes are more reliable for public inbound email consumption (but do not support sending emails). When using custom domains the domain type must match the inbox type. HTTP inboxes are processed by AWS SES while SMTP inboxes use a custom mail server running at &#x60;mxslurp.click&#x60;.',
        required => '0',
    },
    'virtual_inbox' => {
        data_type => 'boolean',
        description => 'Virtual inbox prevents any outbound emails from being sent. It creates sent email records but will never send real emails to recipients. Great for testing and faking email sending.',
        required => '0',
    },
    'use_short_address' => {
        data_type => 'boolean',
        description => 'Use a shorter email address under 31 characters',
        required => '0',
    },
    'domain_id' => {
        data_type => 'string',
        description => 'ID of custom domain to use for email address.',
        required => '0',
    },
    'domain_name' => {
        data_type => 'string',
        description => 'FQDN domain name for the domain you have verified. Will be appended with a randomly assigned recipient name. Use the &#x60;emailAddress&#x60; option instead to specify the full custom inbox.',
        required => '0',
    },
    'prefix' => {
        data_type => 'string',
        description => 'Prefix to add before the email address for easier labelling or identification.',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'create_inbox' } = { 
        summary => 'Create an inbox email address. An inbox has a real email address and can send and receive emails. Inboxes can be either &#x60;SMTP&#x60; or &#x60;HTTP&#x60; inboxes.',
        params => $params,
        returns => 'InboxDto',
        };
}
# @return InboxDto
#
sub create_inbox {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'email_address'}) {
        $query_params->{'emailAddress'} = $self->{api_client}->to_query_value($args{'email_address'});
    }

    # query params
    if ( exists $args{'tags'}) {
        $query_params->{'tags'} = $self->{api_client}->to_query_value($args{'tags'});
    }

    # query params
    if ( exists $args{'name'}) {
        $query_params->{'name'} = $self->{api_client}->to_query_value($args{'name'});
    }

    # query params
    if ( exists $args{'description'}) {
        $query_params->{'description'} = $self->{api_client}->to_query_value($args{'description'});
    }

    # query params
    if ( exists $args{'use_domain_pool'}) {
        $query_params->{'useDomainPool'} = $self->{api_client}->to_query_value($args{'use_domain_pool'});
    }

    # query params
    if ( exists $args{'favourite'}) {
        $query_params->{'favourite'} = $self->{api_client}->to_query_value($args{'favourite'});
    }

    # query params
    if ( exists $args{'expires_at'}) {
        $query_params->{'expiresAt'} = $self->{api_client}->to_query_value($args{'expires_at'});
    }

    # query params
    if ( exists $args{'expires_in'}) {
        $query_params->{'expiresIn'} = $self->{api_client}->to_query_value($args{'expires_in'});
    }

    # query params
    if ( exists $args{'allow_team_access'}) {
        $query_params->{'allowTeamAccess'} = $self->{api_client}->to_query_value($args{'allow_team_access'});
    }

    # query params
    if ( exists $args{'inbox_type'}) {
        $query_params->{'inboxType'} = $self->{api_client}->to_query_value($args{'inbox_type'});
    }

    # query params
    if ( exists $args{'virtual_inbox'}) {
        $query_params->{'virtualInbox'} = $self->{api_client}->to_query_value($args{'virtual_inbox'});
    }

    # query params
    if ( exists $args{'use_short_address'}) {
        $query_params->{'useShortAddress'} = $self->{api_client}->to_query_value($args{'use_short_address'});
    }

    # query params
    if ( exists $args{'domain_id'}) {
        $query_params->{'domainId'} = $self->{api_client}->to_query_value($args{'domain_id'});
    }

    # query params
    if ( exists $args{'domain_name'}) {
        $query_params->{'domainName'} = $self->{api_client}->to_query_value($args{'domain_name'});
    }

    # query params
    if ( exists $args{'prefix'}) {
        $query_params->{'prefix'} = $self->{api_client}->to_query_value($args{'prefix'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('InboxDto', $response);
    return $_response_object;
}

#
# create_inbox_ruleset
#
# Create an inbox ruleset
# 
# @param string $inbox_id inboxId (required)
# @param CreateRulesetOptions $create_ruleset_options  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'inboxId',
        required => '1',
    },
    'create_ruleset_options' => {
        data_type => 'CreateRulesetOptions',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'create_inbox_ruleset' } = { 
        summary => 'Create an inbox ruleset',
        params => $params,
        returns => 'RulesetDto',
        };
}
# @return RulesetDto
#
sub create_inbox_ruleset {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling create_inbox_ruleset");
    }

    # verify the required parameter 'create_ruleset_options' is set
    unless (exists $args{'create_ruleset_options'}) {
      croak("Missing the required parameter 'create_ruleset_options' when calling create_inbox_ruleset");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/rulesets';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # body params
    if ( exists $args{'create_ruleset_options'}) {
        $_body_data = $args{'create_ruleset_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('RulesetDto', $response);
    return $_response_object;
}

#
# create_inbox_with_defaults
#
# Create an inbox with default options. Uses MailSlurp domain pool address and is private.
# 
{
    my $params = {
    };
    __PACKAGE__->method_documentation->{ 'create_inbox_with_defaults' } = { 
        summary => 'Create an inbox with default options. Uses MailSlurp domain pool address and is private.',
        params => $params,
        returns => 'InboxDto',
        };
}
# @return InboxDto
#
sub create_inbox_with_defaults {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/withDefaults';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('InboxDto', $response);
    return $_response_object;
}

#
# create_inbox_with_options
#
# Create an inbox with options. Extended options for inbox creation.
# 
# @param CreateInboxDto $create_inbox_dto  (required)
{
    my $params = {
    'create_inbox_dto' => {
        data_type => 'CreateInboxDto',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'create_inbox_with_options' } = { 
        summary => 'Create an inbox with options. Extended options for inbox creation.',
        params => $params,
        returns => 'InboxDto',
        };
}
# @return InboxDto
#
sub create_inbox_with_options {
    my ($self, %args) = @_;

    # verify the required parameter 'create_inbox_dto' is set
    unless (exists $args{'create_inbox_dto'}) {
      croak("Missing the required parameter 'create_inbox_dto' when calling create_inbox_with_options");
    }

    # parse inputs
    my $_resource_path = '/inboxes/withOptions';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    my $_body_data;
    # body params
    if ( exists $args{'create_inbox_dto'}) {
        $_body_data = $args{'create_inbox_dto'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('InboxDto', $response);
    return $_response_object;
}

#
# delete_all_inbox_emails
#
# Delete all emails in a given inboxes.
# 
# @param string $inbox_id  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'delete_all_inbox_emails' } = { 
        summary => 'Delete all emails in a given inboxes.',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub delete_all_inbox_emails {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling delete_all_inbox_emails");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/deleteAllInboxEmails';

    my $_method = 'DELETE';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# delete_all_inboxes
#
# Delete all inboxes
# 
{
    my $params = {
    };
    __PACKAGE__->method_documentation->{ 'delete_all_inboxes' } = { 
        summary => 'Delete all inboxes',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub delete_all_inboxes {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes';

    my $_method = 'DELETE';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# delete_all_inboxes_by_description
#
# Delete inboxes by description
# 
# @param string $description  (required)
{
    my $params = {
    'description' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'delete_all_inboxes_by_description' } = { 
        summary => 'Delete inboxes by description',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub delete_all_inboxes_by_description {
    my ($self, %args) = @_;

    # verify the required parameter 'description' is set
    unless (exists $args{'description'}) {
      croak("Missing the required parameter 'description' when calling delete_all_inboxes_by_description");
    }

    # parse inputs
    my $_resource_path = '/inboxes/by-description';

    my $_method = 'DELETE';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'description'}) {
        $query_params->{'description'} = $self->{api_client}->to_query_value($args{'description'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# delete_all_inboxes_by_name
#
# Delete inboxes by name
# 
# @param string $name  (required)
{
    my $params = {
    'name' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'delete_all_inboxes_by_name' } = { 
        summary => 'Delete inboxes by name',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub delete_all_inboxes_by_name {
    my ($self, %args) = @_;

    # verify the required parameter 'name' is set
    unless (exists $args{'name'}) {
      croak("Missing the required parameter 'name' when calling delete_all_inboxes_by_name");
    }

    # parse inputs
    my $_resource_path = '/inboxes/by-name';

    my $_method = 'DELETE';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'name'}) {
        $query_params->{'name'} = $self->{api_client}->to_query_value($args{'name'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# delete_all_inboxes_by_tag
#
# Delete inboxes by tag
# 
# @param string $tag  (required)
{
    my $params = {
    'tag' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'delete_all_inboxes_by_tag' } = { 
        summary => 'Delete inboxes by tag',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub delete_all_inboxes_by_tag {
    my ($self, %args) = @_;

    # verify the required parameter 'tag' is set
    unless (exists $args{'tag'}) {
      croak("Missing the required parameter 'tag' when calling delete_all_inboxes_by_tag");
    }

    # parse inputs
    my $_resource_path = '/inboxes/by-tag';

    my $_method = 'DELETE';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'tag'}) {
        $query_params->{'tag'} = $self->{api_client}->to_query_value($args{'tag'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# delete_inbox
#
# Delete inbox
# 
# @param string $inbox_id  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'delete_inbox' } = { 
        summary => 'Delete inbox',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub delete_inbox {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling delete_inbox");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}';

    my $_method = 'DELETE';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# does_inbox_exist
#
# Does inbox exist
# 
# @param string $email_address Email address (required)
# @param boolean $allow_catch_all Allow catch all (optional)
# @param string $ip_address IP address (optional)
# @param string $sender Sender (optional)
{
    my $params = {
    'email_address' => {
        data_type => 'string',
        description => 'Email address',
        required => '1',
    },
    'allow_catch_all' => {
        data_type => 'boolean',
        description => 'Allow catch all',
        required => '0',
    },
    'ip_address' => {
        data_type => 'string',
        description => 'IP address',
        required => '0',
    },
    'sender' => {
        data_type => 'string',
        description => 'Sender',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'does_inbox_exist' } = { 
        summary => 'Does inbox exist',
        params => $params,
        returns => 'InboxExistsDto',
        };
}
# @return InboxExistsDto
#
sub does_inbox_exist {
    my ($self, %args) = @_;

    # verify the required parameter 'email_address' is set
    unless (exists $args{'email_address'}) {
      croak("Missing the required parameter 'email_address' when calling does_inbox_exist");
    }

    # parse inputs
    my $_resource_path = '/inboxes/exists';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'email_address'}) {
        $query_params->{'emailAddress'} = $self->{api_client}->to_query_value($args{'email_address'});
    }

    # query params
    if ( exists $args{'allow_catch_all'}) {
        $query_params->{'allowCatchAll'} = $self->{api_client}->to_query_value($args{'allow_catch_all'});
    }

    # query params
    if ( exists $args{'ip_address'}) {
        $query_params->{'ipAddress'} = $self->{api_client}->to_query_value($args{'ip_address'});
    }

    # query params
    if ( exists $args{'sender'}) {
        $query_params->{'sender'} = $self->{api_client}->to_query_value($args{'sender'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('InboxExistsDto', $response);
    return $_response_object;
}

#
# does_inbox_have_automations
#
# Does inbox have automations
# 
{
    my $params = {
    };
    __PACKAGE__->method_documentation->{ 'does_inbox_have_automations' } = { 
        summary => 'Does inbox have automations',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub does_inbox_have_automations {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/automations';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# flush_expired
#
# Remove expired inboxes
# 
# @param DateTime $before Optional expired at before flag to flush expired inboxes that have expired before the given time (optional)
{
    my $params = {
    'before' => {
        data_type => 'DateTime',
        description => 'Optional expired at before flag to flush expired inboxes that have expired before the given time',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'flush_expired' } = { 
        summary => 'Remove expired inboxes',
        params => $params,
        returns => 'FlushExpiredInboxesResult',
        };
}
# @return FlushExpiredInboxesResult
#
sub flush_expired {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/expired';

    my $_method = 'DELETE';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('FlushExpiredInboxesResult', $response);
    return $_response_object;
}

#
# get_all_inboxes
#
# List All Inboxes Paginated
# 
# @param int $page Optional page index in list pagination (optional, default to 0)
# @param int $size Optional page size in list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param boolean $favourite Optionally filter results for favourites only (optional, default to false)
# @param string $search Optionally filter by search words partial matching name and email address (optional)
# @param string $tag Optionally filter by tags. Will return inboxes that include given tags (optional)
# @param boolean $team_access DEPRECATED. Optionally filter by team access. (optional)
# @param DateTime $since Optional filter by created after given date time (optional)
# @param DateTime $before Optional filter by created before given date time (optional)
# @param string $inbox_type Optional filter by inbox type (optional)
# @param string $inbox_function Optional filter by inbox function (optional)
# @param string $domain_id Optional domain ID filter (optional)
{
    my $params = {
    'page' => {
        data_type => 'int',
        description => 'Optional page index in list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'favourite' => {
        data_type => 'boolean',
        description => 'Optionally filter results for favourites only',
        required => '0',
    },
    'search' => {
        data_type => 'string',
        description => 'Optionally filter by search words partial matching name and email address',
        required => '0',
    },
    'tag' => {
        data_type => 'string',
        description => 'Optionally filter by tags. Will return inboxes that include given tags',
        required => '0',
    },
    'team_access' => {
        data_type => 'boolean',
        description => 'DEPRECATED. Optionally filter by team access.',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Optional filter by created after given date time',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Optional filter by created before given date time',
        required => '0',
    },
    'inbox_type' => {
        data_type => 'string',
        description => 'Optional filter by inbox type',
        required => '0',
    },
    'inbox_function' => {
        data_type => 'string',
        description => 'Optional filter by inbox function',
        required => '0',
    },
    'domain_id' => {
        data_type => 'string',
        description => 'Optional domain ID filter',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_all_inboxes' } = { 
        summary => 'List All Inboxes Paginated',
        params => $params,
        returns => 'PageInboxProjection',
        };
}
# @return PageInboxProjection
#
sub get_all_inboxes {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/paginated';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'favourite'}) {
        $query_params->{'favourite'} = $self->{api_client}->to_query_value($args{'favourite'});
    }

    # query params
    if ( exists $args{'search'}) {
        $query_params->{'search'} = $self->{api_client}->to_query_value($args{'search'});
    }

    # query params
    if ( exists $args{'tag'}) {
        $query_params->{'tag'} = $self->{api_client}->to_query_value($args{'tag'});
    }

    # query params
    if ( exists $args{'team_access'}) {
        $query_params->{'teamAccess'} = $self->{api_client}->to_query_value($args{'team_access'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # query params
    if ( exists $args{'inbox_type'}) {
        $query_params->{'inboxType'} = $self->{api_client}->to_query_value($args{'inbox_type'});
    }

    # query params
    if ( exists $args{'inbox_function'}) {
        $query_params->{'inboxFunction'} = $self->{api_client}->to_query_value($args{'inbox_function'});
    }

    # query params
    if ( exists $args{'domain_id'}) {
        $query_params->{'domainId'} = $self->{api_client}->to_query_value($args{'domain_id'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageInboxProjection', $response);
    return $_response_object;
}

#
# get_all_inboxes_offset_paginated
#
# List All Inboxes Offset Paginated
# 
# @param int $page Optional page index in list pagination (optional, default to 0)
# @param int $size Optional page size in list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param boolean $favourite Optionally filter results for favourites only (optional, default to false)
# @param string $search Optionally filter by search words partial matching name and email address (optional)
# @param string $tag Optionally filter by tags. Will return inboxes that include given tags (optional)
# @param boolean $team_access DEPRECATED. Optionally filter by team access. (optional)
# @param DateTime $since Optional filter by created after given date time (optional)
# @param DateTime $before Optional filter by created before given date time (optional)
# @param string $inbox_type Optional filter by inbox type (optional)
# @param string $inbox_function Optional filter by inbox function (optional)
# @param string $domain_id Optional domain ID filter (optional)
{
    my $params = {
    'page' => {
        data_type => 'int',
        description => 'Optional page index in list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'favourite' => {
        data_type => 'boolean',
        description => 'Optionally filter results for favourites only',
        required => '0',
    },
    'search' => {
        data_type => 'string',
        description => 'Optionally filter by search words partial matching name and email address',
        required => '0',
    },
    'tag' => {
        data_type => 'string',
        description => 'Optionally filter by tags. Will return inboxes that include given tags',
        required => '0',
    },
    'team_access' => {
        data_type => 'boolean',
        description => 'DEPRECATED. Optionally filter by team access.',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Optional filter by created after given date time',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Optional filter by created before given date time',
        required => '0',
    },
    'inbox_type' => {
        data_type => 'string',
        description => 'Optional filter by inbox type',
        required => '0',
    },
    'inbox_function' => {
        data_type => 'string',
        description => 'Optional filter by inbox function',
        required => '0',
    },
    'domain_id' => {
        data_type => 'string',
        description => 'Optional domain ID filter',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_all_inboxes_offset_paginated' } = { 
        summary => 'List All Inboxes Offset Paginated',
        params => $params,
        returns => 'PageInboxProjection',
        };
}
# @return PageInboxProjection
#
sub get_all_inboxes_offset_paginated {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/offset-paginated';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'favourite'}) {
        $query_params->{'favourite'} = $self->{api_client}->to_query_value($args{'favourite'});
    }

    # query params
    if ( exists $args{'search'}) {
        $query_params->{'search'} = $self->{api_client}->to_query_value($args{'search'});
    }

    # query params
    if ( exists $args{'tag'}) {
        $query_params->{'tag'} = $self->{api_client}->to_query_value($args{'tag'});
    }

    # query params
    if ( exists $args{'team_access'}) {
        $query_params->{'teamAccess'} = $self->{api_client}->to_query_value($args{'team_access'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # query params
    if ( exists $args{'inbox_type'}) {
        $query_params->{'inboxType'} = $self->{api_client}->to_query_value($args{'inbox_type'});
    }

    # query params
    if ( exists $args{'inbox_function'}) {
        $query_params->{'inboxFunction'} = $self->{api_client}->to_query_value($args{'inbox_function'});
    }

    # query params
    if ( exists $args{'domain_id'}) {
        $query_params->{'domainId'} = $self->{api_client}->to_query_value($args{'domain_id'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageInboxProjection', $response);
    return $_response_object;
}

#
# get_all_plus_addresses
#
# Get all sub address plus address aliases for an inbox
# 
# @param int $page Optional page index in inbox tracking pixel list pagination (optional, default to 0)
# @param int $size Optional page size in inbox tracking pixel list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param string $inbox_id Optional inboxId filter (optional)
{
    my $params = {
    'page' => {
        data_type => 'int',
        description => 'Optional page index in inbox tracking pixel list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in inbox tracking pixel list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'inbox_id' => {
        data_type => 'string',
        description => 'Optional inboxId filter',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_all_plus_addresses' } = { 
        summary => 'Get all sub address plus address aliases for an inbox',
        params => $params,
        returns => 'PagePlusAddressProjection',
        };
}
# @return PagePlusAddressProjection
#
sub get_all_plus_addresses {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/plus-addresses';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'inbox_id'}) {
        $query_params->{'inboxId'} = $self->{api_client}->to_query_value($args{'inbox_id'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PagePlusAddressProjection', $response);
    return $_response_object;
}

#
# get_all_scheduled_jobs
#
# Get all scheduled email sending jobs for account
# 
# @param int $page Optional page index in scheduled job list pagination (optional, default to 0)
# @param int $size Optional page size in scheduled job list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param DateTime $since Filter by created at after the given timestamp (optional)
# @param DateTime $before Filter by created at before the given timestamp (optional)
# @param string $inbox_id  (optional)
{
    my $params = {
    'page' => {
        data_type => 'int',
        description => 'Optional page index in scheduled job list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in scheduled job list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Filter by created at after the given timestamp',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Filter by created at before the given timestamp',
        required => '0',
    },
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_all_scheduled_jobs' } = { 
        summary => 'Get all scheduled email sending jobs for account',
        params => $params,
        returns => 'PageScheduledJobs',
        };
}
# @return PageScheduledJobs
#
sub get_all_scheduled_jobs {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/scheduled-jobs';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # query params
    if ( exists $args{'inbox_id'}) {
        $query_params->{'inboxId'} = $self->{api_client}->to_query_value($args{'inbox_id'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageScheduledJobs', $response);
    return $_response_object;
}

#
# get_delivery_statuses_by_inbox_id
#
# 
# 
# @param string $inbox_id  (required)
# @param int $page Optional page index in delivery status list pagination (optional, default to 0)
# @param int $size Optional page size in delivery status list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param DateTime $since Filter by created at after the given timestamp (optional)
# @param DateTime $before Filter by created at before the given timestamp (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'page' => {
        data_type => 'int',
        description => 'Optional page index in delivery status list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in delivery status list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Filter by created at after the given timestamp',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Filter by created at before the given timestamp',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_delivery_statuses_by_inbox_id' } = { 
        summary => '',
        params => $params,
        returns => 'PageDeliveryStatus',
        };
}
# @return PageDeliveryStatus
#
sub get_delivery_statuses_by_inbox_id {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_delivery_statuses_by_inbox_id");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/delivery-status';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageDeliveryStatus', $response);
    return $_response_object;
}

#
# get_emails
#
# Get emails in an Inbox. This method is not idempotent as it allows retries and waits if you want certain conditions to be met before returning. For simple listing and sorting of known emails use the email controller instead.
# 
# @param string $inbox_id Id of inbox that emails belongs to (required)
# @param int $size Alias for limit. Assessed first before assessing any passed limit. (optional)
# @param int $limit Limit the result set, ordered by received date time sort direction. Maximum 100. For more listing options see the email controller (optional)
# @param string $sort Sort the results by received date and direction ASC or DESC (optional)
# @param int $retry_timeout Maximum milliseconds to spend retrying inbox database until minCount emails are returned (optional)
# @param int $delay_timeout  (optional)
# @param int $min_count Minimum acceptable email count. Will cause request to hang (and retry) until minCount is satisfied or retryTimeout is reached. (optional)
# @param boolean $unread_only  (optional)
# @param DateTime $before Exclude emails received after this ISO 8601 date time (optional)
# @param DateTime $since Exclude emails received before this ISO 8601 date time (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'Id of inbox that emails belongs to',
        required => '1',
    },
    'size' => {
        data_type => 'int',
        description => 'Alias for limit. Assessed first before assessing any passed limit.',
        required => '0',
    },
    'limit' => {
        data_type => 'int',
        description => 'Limit the result set, ordered by received date time sort direction. Maximum 100. For more listing options see the email controller',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Sort the results by received date and direction ASC or DESC',
        required => '0',
    },
    'retry_timeout' => {
        data_type => 'int',
        description => 'Maximum milliseconds to spend retrying inbox database until minCount emails are returned',
        required => '0',
    },
    'delay_timeout' => {
        data_type => 'int',
        description => '',
        required => '0',
    },
    'min_count' => {
        data_type => 'int',
        description => 'Minimum acceptable email count. Will cause request to hang (and retry) until minCount is satisfied or retryTimeout is reached.',
        required => '0',
    },
    'unread_only' => {
        data_type => 'boolean',
        description => '',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Exclude emails received after this ISO 8601 date time',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Exclude emails received before this ISO 8601 date time',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_emails' } = { 
        summary => 'Get emails in an Inbox. This method is not idempotent as it allows retries and waits if you want certain conditions to be met before returning. For simple listing and sorting of known emails use the email controller instead.',
        params => $params,
        returns => 'ARRAY[EmailPreview]',
        };
}
# @return ARRAY[EmailPreview]
#
sub get_emails {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_emails");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/emails';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'limit'}) {
        $query_params->{'limit'} = $self->{api_client}->to_query_value($args{'limit'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'retry_timeout'}) {
        $query_params->{'retryTimeout'} = $self->{api_client}->to_query_value($args{'retry_timeout'});
    }

    # query params
    if ( exists $args{'delay_timeout'}) {
        $query_params->{'delayTimeout'} = $self->{api_client}->to_query_value($args{'delay_timeout'});
    }

    # query params
    if ( exists $args{'min_count'}) {
        $query_params->{'minCount'} = $self->{api_client}->to_query_value($args{'min_count'});
    }

    # query params
    if ( exists $args{'unread_only'}) {
        $query_params->{'unreadOnly'} = $self->{api_client}->to_query_value($args{'unread_only'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('ARRAY[EmailPreview]', $response);
    return $_response_object;
}

#
# get_imap_access
#
# 
# 
# @param string $inbox_id Inbox ID (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'Inbox ID',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_imap_access' } = { 
        summary => '',
        params => $params,
        returns => 'ImapAccessDetails',
        };
}
# @return ImapAccessDetails
#
sub get_imap_access {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/imap-access';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'inbox_id'}) {
        $query_params->{'inboxId'} = $self->{api_client}->to_query_value($args{'inbox_id'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('ImapAccessDetails', $response);
    return $_response_object;
}

#
# get_imap_smtp_access
#
# 
# 
# @param string $inbox_id Inbox ID (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'Inbox ID',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_imap_smtp_access' } = { 
        summary => '',
        params => $params,
        returns => 'ImapSmtpAccessDetails',
        };
}
# @return ImapSmtpAccessDetails
#
sub get_imap_smtp_access {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/imap-smtp-access';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'inbox_id'}) {
        $query_params->{'inboxId'} = $self->{api_client}->to_query_value($args{'inbox_id'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('ImapSmtpAccessDetails', $response);
    return $_response_object;
}

#
# get_imap_smtp_access_env
#
# 
# 
# @param string $inbox_id Inbox ID (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'Inbox ID',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_imap_smtp_access_env' } = { 
        summary => '',
        params => $params,
        returns => 'string',
        };
}
# @return string
#
sub get_imap_smtp_access_env {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/imap-smtp-access/env';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'inbox_id'}) {
        $query_params->{'inboxId'} = $self->{api_client}->to_query_value($args{'inbox_id'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('string', $response);
    return $_response_object;
}

#
# get_imap_smtp_access_servers
#
# 
# 
{
    my $params = {
    };
    __PACKAGE__->method_documentation->{ 'get_imap_smtp_access_servers' } = { 
        summary => '',
        params => $params,
        returns => 'ImapSmtpAccessServers',
        };
}
# @return ImapSmtpAccessServers
#
sub get_imap_smtp_access_servers {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/imap-smtp-access/servers';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('ImapSmtpAccessServers', $response);
    return $_response_object;
}

#
# get_inbox
#
# Get Inbox. Returns properties of an inbox.
# 
# @param string $inbox_id  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox' } = { 
        summary => 'Get Inbox. Returns properties of an inbox.',
        params => $params,
        returns => 'InboxDto',
        };
}
# @return InboxDto
#
sub get_inbox {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_inbox");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('InboxDto', $response);
    return $_response_object;
}

#
# get_inbox_by_email_address
#
# Search for an inbox with the provided email address
# 
# @param string $email_address  (required)
{
    my $params = {
    'email_address' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_by_email_address' } = { 
        summary => 'Search for an inbox with the provided email address',
        params => $params,
        returns => 'InboxByEmailAddressResult',
        };
}
# @return InboxByEmailAddressResult
#
sub get_inbox_by_email_address {
    my ($self, %args) = @_;

    # verify the required parameter 'email_address' is set
    unless (exists $args{'email_address'}) {
      croak("Missing the required parameter 'email_address' when calling get_inbox_by_email_address");
    }

    # parse inputs
    my $_resource_path = '/inboxes/byEmailAddress';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'email_address'}) {
        $query_params->{'emailAddress'} = $self->{api_client}->to_query_value($args{'email_address'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('InboxByEmailAddressResult', $response);
    return $_response_object;
}

#
# get_inbox_by_name
#
# Search for an inbox with the given name
# 
# @param string $name  (required)
{
    my $params = {
    'name' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_by_name' } = { 
        summary => 'Search for an inbox with the given name',
        params => $params,
        returns => 'InboxByNameResult',
        };
}
# @return InboxByNameResult
#
sub get_inbox_by_name {
    my ($self, %args) = @_;

    # verify the required parameter 'name' is set
    unless (exists $args{'name'}) {
      croak("Missing the required parameter 'name' when calling get_inbox_by_name");
    }

    # parse inputs
    my $_resource_path = '/inboxes/byName';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'name'}) {
        $query_params->{'name'} = $self->{api_client}->to_query_value($args{'name'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('InboxByNameResult', $response);
    return $_response_object;
}

#
# get_inbox_count
#
# Get total inbox count
# 
{
    my $params = {
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_count' } = { 
        summary => 'Get total inbox count',
        params => $params,
        returns => 'CountDto',
        };
}
# @return CountDto
#
sub get_inbox_count {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/count';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('CountDto', $response);
    return $_response_object;
}

#
# get_inbox_email_count
#
# Get email count in inbox
# 
# @param string $inbox_id Id of inbox that emails belongs to (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'Id of inbox that emails belongs to',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_email_count' } = { 
        summary => 'Get email count in inbox',
        params => $params,
        returns => 'CountDto',
        };
}
# @return CountDto
#
sub get_inbox_email_count {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_inbox_email_count");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/emails/count';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('CountDto', $response);
    return $_response_object;
}

#
# get_inbox_emails_paginated
#
# Get inbox emails paginated
# 
# @param string $inbox_id Id of inbox that emails belongs to (required)
# @param int $page Optional page index in inbox emails list pagination (optional, default to 0)
# @param int $size Optional page size in inbox emails list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param DateTime $since Optional filter by received after given date time (optional)
# @param DateTime $before Optional filter by received before given date time (optional)
# @param boolean $sync_connectors Sync connectors before fetching emails (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'Id of inbox that emails belongs to',
        required => '1',
    },
    'page' => {
        data_type => 'int',
        description => 'Optional page index in inbox emails list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in inbox emails list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Optional filter by received after given date time',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Optional filter by received before given date time',
        required => '0',
    },
    'sync_connectors' => {
        data_type => 'boolean',
        description => 'Sync connectors before fetching emails',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_emails_paginated' } = { 
        summary => 'Get inbox emails paginated',
        params => $params,
        returns => 'PageEmailPreview',
        };
}
# @return PageEmailPreview
#
sub get_inbox_emails_paginated {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_inbox_emails_paginated");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/emails/paginated';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # query params
    if ( exists $args{'sync_connectors'}) {
        $query_params->{'syncConnectors'} = $self->{api_client}->to_query_value($args{'sync_connectors'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageEmailPreview', $response);
    return $_response_object;
}

#
# get_inbox_ids
#
# Get all inbox IDs
# 
{
    my $params = {
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_ids' } = { 
        summary => 'Get all inbox IDs',
        params => $params,
        returns => 'InboxIdsResult',
        };
}
# @return InboxIdsResult
#
sub get_inbox_ids {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/ids';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('InboxIdsResult', $response);
    return $_response_object;
}

#
# get_inbox_plus_address
#
# Get sub address plus address for an inbox
# 
# @param string $plus_address_id ID of the plus address you want to fetch (required)
# @param string $inbox_id ID of the inbox you want to fetch (required)
{
    my $params = {
    'plus_address_id' => {
        data_type => 'string',
        description => 'ID of the plus address you want to fetch',
        required => '1',
    },
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to fetch',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_plus_address' } = { 
        summary => 'Get sub address plus address for an inbox',
        params => $params,
        returns => 'PlusAddressDto',
        };
}
# @return PlusAddressDto
#
sub get_inbox_plus_address {
    my ($self, %args) = @_;

    # verify the required parameter 'plus_address_id' is set
    unless (exists $args{'plus_address_id'}) {
      croak("Missing the required parameter 'plus_address_id' when calling get_inbox_plus_address");
    }

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_inbox_plus_address");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/plus-addresses/{plusAddressId}';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # path params
    if ( exists $args{'plus_address_id'}) {
        my $_base_variable = "{" . "plusAddressId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'plus_address_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PlusAddressDto', $response);
    return $_response_object;
}

#
# get_inbox_plus_address_by_id
#
# Get sub address plus address by ID
# 
# @param string $plus_address_id ID of the plus address you want to fetch (required)
# @param string $inbox_id ID of the inbox you want to filter for (optional)
{
    my $params = {
    'plus_address_id' => {
        data_type => 'string',
        description => 'ID of the plus address you want to fetch',
        required => '1',
    },
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to filter for',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_plus_address_by_id' } = { 
        summary => 'Get sub address plus address by ID',
        params => $params,
        returns => 'PlusAddressDto',
        };
}
# @return PlusAddressDto
#
sub get_inbox_plus_address_by_id {
    my ($self, %args) = @_;

    # verify the required parameter 'plus_address_id' is set
    unless (exists $args{'plus_address_id'}) {
      croak("Missing the required parameter 'plus_address_id' when calling get_inbox_plus_address_by_id");
    }

    # parse inputs
    my $_resource_path = '/inboxes/plus-addresses/{plusAddressId}';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'inbox_id'}) {
        $query_params->{'inboxId'} = $self->{api_client}->to_query_value($args{'inbox_id'});
    }

    # path params
    if ( exists $args{'plus_address_id'}) {
        my $_base_variable = "{" . "plusAddressId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'plus_address_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PlusAddressDto', $response);
    return $_response_object;
}

#
# get_inbox_plus_address_emails
#
# Get emails for a given inbox plus address
# 
# @param string $plus_address The plus address to fetch emails for. If your inbox address is &#x60;123@test.com&#x60; and the email was sent to &#x60;123+abc@test.com&#x60; then the plus address is &#x60;abc&#x60; (required)
# @param string $inbox_id ID of the inbox you want to send the email from (required)
# @param int $page Optional page index in inbox tracking pixel list pagination (optional, default to 0)
# @param int $size Optional page size in inbox tracking pixel list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param DateTime $since Optional filter by created after given date time (optional)
# @param DateTime $before Optional filter by created before given date time (optional)
{
    my $params = {
    'plus_address' => {
        data_type => 'string',
        description => 'The plus address to fetch emails for. If your inbox address is &#x60;123@test.com&#x60; and the email was sent to &#x60;123+abc@test.com&#x60; then the plus address is &#x60;abc&#x60;',
        required => '1',
    },
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to send the email from',
        required => '1',
    },
    'page' => {
        data_type => 'int',
        description => 'Optional page index in inbox tracking pixel list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in inbox tracking pixel list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Optional filter by created after given date time',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Optional filter by created before given date time',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_plus_address_emails' } = { 
        summary => 'Get emails for a given inbox plus address',
        params => $params,
        returns => 'PageEmailPreview',
        };
}
# @return PageEmailPreview
#
sub get_inbox_plus_address_emails {
    my ($self, %args) = @_;

    # verify the required parameter 'plus_address' is set
    unless (exists $args{'plus_address'}) {
      croak("Missing the required parameter 'plus_address' when calling get_inbox_plus_address_emails");
    }

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_inbox_plus_address_emails");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/plus-addresses/emails';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'plus_address'}) {
        $query_params->{'plusAddress'} = $self->{api_client}->to_query_value($args{'plus_address'});
    }

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageEmailPreview', $response);
    return $_response_object;
}

#
# get_inbox_plus_address_emails_for_plus_address_id
#
# Get emails for a given inbox plus address
# 
# @param string $plus_address_id The plus address ID to fetch emails for. (required)
# @param string $inbox_id ID of the inbox you want to send the email from (required)
# @param int $page Optional page index in inbox tracking pixel list pagination (optional, default to 0)
# @param int $size Optional page size in inbox tracking pixel list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param DateTime $since Optional filter by created after given date time (optional)
# @param DateTime $before Optional filter by created before given date time (optional)
{
    my $params = {
    'plus_address_id' => {
        data_type => 'string',
        description => 'The plus address ID to fetch emails for.',
        required => '1',
    },
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to send the email from',
        required => '1',
    },
    'page' => {
        data_type => 'int',
        description => 'Optional page index in inbox tracking pixel list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in inbox tracking pixel list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Optional filter by created after given date time',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Optional filter by created before given date time',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_plus_address_emails_for_plus_address_id' } = { 
        summary => 'Get emails for a given inbox plus address',
        params => $params,
        returns => 'PageEmailPreview',
        };
}
# @return PageEmailPreview
#
sub get_inbox_plus_address_emails_for_plus_address_id {
    my ($self, %args) = @_;

    # verify the required parameter 'plus_address_id' is set
    unless (exists $args{'plus_address_id'}) {
      croak("Missing the required parameter 'plus_address_id' when calling get_inbox_plus_address_emails_for_plus_address_id");
    }

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_inbox_plus_address_emails_for_plus_address_id");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/plus-addresses/{plusAddressId}/emails';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # path params
    if ( exists $args{'plus_address_id'}) {
        my $_base_variable = "{" . "plusAddressId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'plus_address_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageEmailPreview', $response);
    return $_response_object;
}

#
# get_inbox_plus_addresses
#
# Get sub address plus address aliases for an inbox
# 
# @param string $inbox_id ID of the inbox you want to send the email from (required)
# @param int $page Optional page index in inbox tracking pixel list pagination (optional, default to 0)
# @param int $size Optional page size in inbox tracking pixel list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to send the email from',
        required => '1',
    },
    'page' => {
        data_type => 'int',
        description => 'Optional page index in inbox tracking pixel list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in inbox tracking pixel list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_plus_addresses' } = { 
        summary => 'Get sub address plus address aliases for an inbox',
        params => $params,
        returns => 'PagePlusAddressProjection',
        };
}
# @return PagePlusAddressProjection
#
sub get_inbox_plus_addresses {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_inbox_plus_addresses");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/plus-addresses';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PagePlusAddressProjection', $response);
    return $_response_object;
}

#
# get_inbox_sent_count
#
# Get sent email count in inbox
# 
# @param string $inbox_id Id of inbox that emails were sent from (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'Id of inbox that emails were sent from',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_sent_count' } = { 
        summary => 'Get sent email count in inbox',
        params => $params,
        returns => 'CountDto',
        };
}
# @return CountDto
#
sub get_inbox_sent_count {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_inbox_sent_count");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/sent/count';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('CountDto', $response);
    return $_response_object;
}

#
# get_inbox_sent_emails
#
# Get Inbox Sent Emails
# 
# @param string $inbox_id  (required)
# @param int $page Optional page index in inbox sent email list pagination (optional, default to 0)
# @param int $size Optional page size in inbox sent email list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param string $search_filter Optional sent email search (optional)
# @param DateTime $since Optional filter by sent after given date time (optional)
# @param DateTime $before Optional filter by sent before given date time (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'page' => {
        data_type => 'int',
        description => 'Optional page index in inbox sent email list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in inbox sent email list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'search_filter' => {
        data_type => 'string',
        description => 'Optional sent email search',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Optional filter by sent after given date time',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Optional filter by sent before given date time',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_sent_emails' } = { 
        summary => 'Get Inbox Sent Emails',
        params => $params,
        returns => 'PageSentEmailProjection',
        };
}
# @return PageSentEmailProjection
#
sub get_inbox_sent_emails {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_inbox_sent_emails");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/sent';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'search_filter'}) {
        $query_params->{'searchFilter'} = $self->{api_client}->to_query_value($args{'search_filter'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageSentEmailProjection', $response);
    return $_response_object;
}

#
# get_inbox_tags
#
# Get inbox tags
# 
# @param int $page Optional page index in list pagination (optional, default to 0)
# @param int $size Optional page size in list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param string $search_filter Optional search filter (optional)
{
    my $params = {
    'page' => {
        data_type => 'int',
        description => 'Optional page index in list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'search_filter' => {
        data_type => 'string',
        description => 'Optional search filter',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_tags' } = { 
        summary => 'Get inbox tags',
        params => $params,
        returns => 'ARRAY[string]',
        };
}
# @return ARRAY[string]
#
sub get_inbox_tags {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/tags';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'search_filter'}) {
        $query_params->{'searchFilter'} = $self->{api_client}->to_query_value($args{'search_filter'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('ARRAY[string]', $response);
    return $_response_object;
}

#
# get_inbox_tags_paginated
#
# Get inbox tags paginated
# 
# @param int $page Optional page index in list pagination (optional, default to 0)
# @param int $size Optional page size in list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param string $search_filter Optional search filter (optional)
{
    my $params = {
    'page' => {
        data_type => 'int',
        description => 'Optional page index in list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'search_filter' => {
        data_type => 'string',
        description => 'Optional search filter',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inbox_tags_paginated' } = { 
        summary => 'Get inbox tags paginated',
        params => $params,
        returns => 'PageInboxTags',
        };
}
# @return PageInboxTags
#
sub get_inbox_tags_paginated {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/tags/paginated';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'search_filter'}) {
        $query_params->{'searchFilter'} = $self->{api_client}->to_query_value($args{'search_filter'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageInboxTags', $response);
    return $_response_object;
}

#
# get_inboxes
#
# List Inboxes and email addresses
# 
# @param int $size Optional result size limit. Note an automatic limit of 100 results is applied. See the paginated &#x60;getAllEmails&#x60; for larger queries. (optional, default to 100)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param DateTime $since Optional filter by created after given date time (optional)
# @param boolean $exclude_catch_all_inboxes Optional exclude catch all inboxes (optional)
# @param DateTime $before Optional filter by created before given date time (optional)
# @param ARRAY[string] $include Optional inboxIds to include in result (optional)
{
    my $params = {
    'size' => {
        data_type => 'int',
        description => 'Optional result size limit. Note an automatic limit of 100 results is applied. See the paginated &#x60;getAllEmails&#x60; for larger queries.',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Optional filter by created after given date time',
        required => '0',
    },
    'exclude_catch_all_inboxes' => {
        data_type => 'boolean',
        description => 'Optional exclude catch all inboxes',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Optional filter by created before given date time',
        required => '0',
    },
    'include' => {
        data_type => 'ARRAY[string]',
        description => 'Optional inboxIds to include in result',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inboxes' } = { 
        summary => 'List Inboxes and email addresses',
        params => $params,
        returns => 'ARRAY[InboxDto]',
        };
}
# @return ARRAY[InboxDto]
#
sub get_inboxes {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'exclude_catch_all_inboxes'}) {
        $query_params->{'excludeCatchAllInboxes'} = $self->{api_client}->to_query_value($args{'exclude_catch_all_inboxes'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # query params
    if ( exists $args{'include'}) {
        $query_params->{'include'} = $self->{api_client}->to_query_value($args{'include'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('ARRAY[InboxDto]', $response);
    return $_response_object;
}

#
# get_inboxes_by_tag
#
# Get inboxes for a tag
# 
# @param string $tag Tag to filter by (required)
# @param int $page Optional page index in list pagination (optional, default to 0)
# @param int $size Optional page size in list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param string $search_filter Optional search filter (optional)
{
    my $params = {
    'tag' => {
        data_type => 'string',
        description => 'Tag to filter by',
        required => '1',
    },
    'page' => {
        data_type => 'int',
        description => 'Optional page index in list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'search_filter' => {
        data_type => 'string',
        description => 'Optional search filter',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_inboxes_by_tag' } = { 
        summary => 'Get inboxes for a tag',
        params => $params,
        returns => 'PageInboxProjection',
        };
}
# @return PageInboxProjection
#
sub get_inboxes_by_tag {
    my ($self, %args) = @_;

    # verify the required parameter 'tag' is set
    unless (exists $args{'tag'}) {
      croak("Missing the required parameter 'tag' when calling get_inboxes_by_tag");
    }

    # parse inputs
    my $_resource_path = '/inboxes/tags/inboxes';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'search_filter'}) {
        $query_params->{'searchFilter'} = $self->{api_client}->to_query_value($args{'search_filter'});
    }

    # query params
    if ( exists $args{'tag'}) {
        $query_params->{'tag'} = $self->{api_client}->to_query_value($args{'tag'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageInboxProjection', $response);
    return $_response_object;
}

#
# get_latest_email_in_inbox
#
# Get latest email in an inbox. Use `WaitForController` to get emails that may not have arrived yet.
# 
# @param string $inbox_id ID of the inbox you want to get the latest email from (required)
# @param int $timeout_millis Timeout milliseconds to wait for latest email (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to get the latest email from',
        required => '1',
    },
    'timeout_millis' => {
        data_type => 'int',
        description => 'Timeout milliseconds to wait for latest email',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_latest_email_in_inbox' } = { 
        summary => 'Get latest email in an inbox. Use &#x60;WaitForController&#x60; to get emails that may not have arrived yet.',
        params => $params,
        returns => 'Email',
        };
}
# @return Email
#
sub get_latest_email_in_inbox {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_latest_email_in_inbox");
    }

    # verify the required parameter 'timeout_millis' is set
    unless (exists $args{'timeout_millis'}) {
      croak("Missing the required parameter 'timeout_millis' when calling get_latest_email_in_inbox");
    }

    # parse inputs
    my $_resource_path = '/inboxes/getLatestEmail';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'inbox_id'}) {
        $query_params->{'inboxId'} = $self->{api_client}->to_query_value($args{'inbox_id'});
    }

    # query params
    if ( exists $args{'timeout_millis'}) {
        $query_params->{'timeoutMillis'} = $self->{api_client}->to_query_value($args{'timeout_millis'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('Email', $response);
    return $_response_object;
}

#
# get_or_create_inbox_plus_address
#
# Get or create a plus address by full address
# 
# @param string $inbox_id ID of the inbox you want to target (required)
# @param string $full_address Full recipient address including +suffix. Example: &#x60;inbox+alias@domain.com&#x60; (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to target',
        required => '1',
    },
    'full_address' => {
        data_type => 'string',
        description => 'Full recipient address including +suffix. Example: &#x60;inbox+alias@domain.com&#x60;',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_or_create_inbox_plus_address' } = { 
        summary => 'Get or create a plus address by full address',
        params => $params,
        returns => 'PlusAddressDto',
        };
}
# @return PlusAddressDto
#
sub get_or_create_inbox_plus_address {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_or_create_inbox_plus_address");
    }

    # verify the required parameter 'full_address' is set
    unless (exists $args{'full_address'}) {
      croak("Missing the required parameter 'full_address' when calling get_or_create_inbox_plus_address");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/plus-addresses/get-or-create';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'full_address'}) {
        $query_params->{'fullAddress'} = $self->{api_client}->to_query_value($args{'full_address'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PlusAddressDto', $response);
    return $_response_object;
}

#
# get_or_create_plus_address_by_full_address
#
# Get or create a plus address by full address without inbox ID
# 
# @param string $full_address Full recipient address including +suffix. Example: &#x60;inbox+alias@domain.com&#x60; (required)
{
    my $params = {
    'full_address' => {
        data_type => 'string',
        description => 'Full recipient address including +suffix. Example: &#x60;inbox+alias@domain.com&#x60;',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_or_create_plus_address_by_full_address' } = { 
        summary => 'Get or create a plus address by full address without inbox ID',
        params => $params,
        returns => 'PlusAddressDto',
        };
}
# @return PlusAddressDto
#
sub get_or_create_plus_address_by_full_address {
    my ($self, %args) = @_;

    # verify the required parameter 'full_address' is set
    unless (exists $args{'full_address'}) {
      croak("Missing the required parameter 'full_address' when calling get_or_create_plus_address_by_full_address");
    }

    # parse inputs
    my $_resource_path = '/inboxes/get-or-create-plus-address';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'full_address'}) {
        $query_params->{'fullAddress'} = $self->{api_client}->to_query_value($args{'full_address'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PlusAddressDto', $response);
    return $_response_object;
}

#
# get_organization_inboxes
#
# List Organization Inboxes Paginated
# 
# @param int $page Optional page index in list pagination (optional, default to 0)
# @param int $size Optional page size in list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param string $search_filter Optional search filter (optional)
# @param DateTime $since Optional filter by created after given date time (optional)
# @param DateTime $before Optional filter by created before given date time (optional)
{
    my $params = {
    'page' => {
        data_type => 'int',
        description => 'Optional page index in list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'search_filter' => {
        data_type => 'string',
        description => 'Optional search filter',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Optional filter by created after given date time',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Optional filter by created before given date time',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_organization_inboxes' } = { 
        summary => 'List Organization Inboxes Paginated',
        params => $params,
        returns => 'PageOrganizationInboxProjection',
        };
}
# @return PageOrganizationInboxProjection
#
sub get_organization_inboxes {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/organization';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'search_filter'}) {
        $query_params->{'searchFilter'} = $self->{api_client}->to_query_value($args{'search_filter'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageOrganizationInboxProjection', $response);
    return $_response_object;
}

#
# get_outboxes
#
# List all inboxes with sent emails
# 
# @param int $page Optional page index in list pagination (optional, default to 0)
# @param int $size Optional page size in list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
{
    my $params = {
    'page' => {
        data_type => 'int',
        description => 'Optional page index in list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_outboxes' } = { 
        summary => 'List all inboxes with sent emails',
        params => $params,
        returns => 'PageInboxProjection',
        };
}
# @return PageInboxProjection
#
sub get_outboxes {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/outboxes';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageInboxProjection', $response);
    return $_response_object;
}

#
# get_scheduled_job
#
# Get a scheduled email job
# 
# @param string $job_id  (required)
{
    my $params = {
    'job_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_scheduled_job' } = { 
        summary => 'Get a scheduled email job',
        params => $params,
        returns => 'ScheduledJobDto',
        };
}
# @return ScheduledJobDto
#
sub get_scheduled_job {
    my ($self, %args) = @_;

    # verify the required parameter 'job_id' is set
    unless (exists $args{'job_id'}) {
      croak("Missing the required parameter 'job_id' when calling get_scheduled_job");
    }

    # parse inputs
    my $_resource_path = '/inboxes/scheduled-jobs/{jobId}';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # path params
    if ( exists $args{'job_id'}) {
        my $_base_variable = "{" . "jobId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'job_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('ScheduledJobDto', $response);
    return $_response_object;
}

#
# get_scheduled_jobs_by_inbox_id
#
# Get all scheduled email sending jobs for the inbox
# 
# @param string $inbox_id  (required)
# @param int $page Optional page index in scheduled job list pagination (optional, default to 0)
# @param int $size Optional page size in scheduled job list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param DateTime $since Filter by created at after the given timestamp (optional)
# @param DateTime $before Filter by created at before the given timestamp (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'page' => {
        data_type => 'int',
        description => 'Optional page index in scheduled job list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in scheduled job list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Filter by created at after the given timestamp',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Filter by created at before the given timestamp',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_scheduled_jobs_by_inbox_id' } = { 
        summary => 'Get all scheduled email sending jobs for the inbox',
        params => $params,
        returns => 'PageScheduledJobs',
        };
}
# @return PageScheduledJobs
#
sub get_scheduled_jobs_by_inbox_id {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling get_scheduled_jobs_by_inbox_id");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/scheduled-jobs';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageScheduledJobs', $response);
    return $_response_object;
}

#
# get_smtp_access
#
# 
# 
# @param string $inbox_id Inbox ID (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'Inbox ID',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'get_smtp_access' } = { 
        summary => '',
        params => $params,
        returns => 'SmtpAccessDetails',
        };
}
# @return SmtpAccessDetails
#
sub get_smtp_access {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/inboxes/smtp-access';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'inbox_id'}) {
        $query_params->{'inboxId'} = $self->{api_client}->to_query_value($args{'inbox_id'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('SmtpAccessDetails', $response);
    return $_response_object;
}

#
# import_email_into_inbox
#
# Import email into inbox from base64 MIME
# 
# @param string $inbox_id  (required)
# @param ImportEmailOptions $import_email_options  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'import_email_options' => {
        data_type => 'ImportEmailOptions',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'import_email_into_inbox' } = { 
        summary => 'Import email into inbox from base64 MIME',
        params => $params,
        returns => 'Email',
        };
}
# @return Email
#
sub import_email_into_inbox {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling import_email_into_inbox");
    }

    # verify the required parameter 'import_email_options' is set
    unless (exists $args{'import_email_options'}) {
      croak("Missing the required parameter 'import_email_options' when calling import_email_into_inbox");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/emails/import';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # body params
    if ( exists $args{'import_email_options'}) {
        $_body_data = $args{'import_email_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('Email', $response);
    return $_response_object;
}

#
# import_email_into_inbox_bytes
#
# Import email into inbox from raw MIME bytes
# 
# @param string $inbox_id  (required)
# @param string $body  (required)
# @param string $external_id  (optional)
# @param boolean $run_pipeline  (optional, default to false)
# @param boolean $override_message_id  (optional, default to true)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'body' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'external_id' => {
        data_type => 'string',
        description => '',
        required => '0',
    },
    'run_pipeline' => {
        data_type => 'boolean',
        description => '',
        required => '0',
    },
    'override_message_id' => {
        data_type => 'boolean',
        description => '',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'import_email_into_inbox_bytes' } = { 
        summary => 'Import email into inbox from raw MIME bytes',
        params => $params,
        returns => 'Email',
        };
}
# @return Email
#
sub import_email_into_inbox_bytes {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling import_email_into_inbox_bytes");
    }

    # verify the required parameter 'body' is set
    unless (exists $args{'body'}) {
      croak("Missing the required parameter 'body' when calling import_email_into_inbox_bytes");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/emails/import/raw';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/octet-stream', 'message/rfc822');

    # query params
    if ( exists $args{'external_id'}) {
        $query_params->{'externalId'} = $self->{api_client}->to_query_value($args{'external_id'});
    }

    # query params
    if ( exists $args{'run_pipeline'}) {
        $query_params->{'runPipeline'} = $self->{api_client}->to_query_value($args{'run_pipeline'});
    }

    # query params
    if ( exists $args{'override_message_id'}) {
        $query_params->{'overrideMessageId'} = $self->{api_client}->to_query_value($args{'override_message_id'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # body params
    if ( exists $args{'body'}) {
        $_body_data = $args{'body'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('Email', $response);
    return $_response_object;
}

#
# import_email_into_inbox_multipart
#
# Import email into inbox from multipart EML upload
# 
# @param string $inbox_id  (required)
# @param string $file  (required)
# @param string $external_id  (optional)
# @param boolean $run_pipeline  (optional, default to false)
# @param boolean $override_message_id  (optional, default to true)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'file' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'external_id' => {
        data_type => 'string',
        description => '',
        required => '0',
    },
    'run_pipeline' => {
        data_type => 'boolean',
        description => '',
        required => '0',
    },
    'override_message_id' => {
        data_type => 'boolean',
        description => '',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'import_email_into_inbox_multipart' } = { 
        summary => 'Import email into inbox from multipart EML upload',
        params => $params,
        returns => 'Email',
        };
}
# @return Email
#
sub import_email_into_inbox_multipart {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling import_email_into_inbox_multipart");
    }

    # verify the required parameter 'file' is set
    unless (exists $args{'file'}) {
      croak("Missing the required parameter 'file' when calling import_email_into_inbox_multipart");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/emails/import/multipart';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('multipart/form-data');

    # query params
    if ( exists $args{'external_id'}) {
        $query_params->{'externalId'} = $self->{api_client}->to_query_value($args{'external_id'});
    }

    # query params
    if ( exists $args{'run_pipeline'}) {
        $query_params->{'runPipeline'} = $self->{api_client}->to_query_value($args{'run_pipeline'});
    }

    # query params
    if ( exists $args{'override_message_id'}) {
        $query_params->{'overrideMessageId'} = $self->{api_client}->to_query_value($args{'override_message_id'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    # form params
    if ( exists $args{'file'} ) {
        $form_params->{'file'} = [] unless defined $form_params->{'file'};
        push @{$form_params->{'file'}}, $args{'file'};
            }
    
    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('Email', $response);
    return $_response_object;
}

#
# is_email_address_available
#
# Is email address available
# 
# @param string $email_address  (required)
{
    my $params = {
    'email_address' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'is_email_address_available' } = { 
        summary => 'Is email address available',
        params => $params,
        returns => 'EmailAvailableResult',
        };
}
# @return EmailAvailableResult
#
sub is_email_address_available {
    my ($self, %args) = @_;

    # verify the required parameter 'email_address' is set
    unless (exists $args{'email_address'}) {
      croak("Missing the required parameter 'email_address' when calling is_email_address_available");
    }

    # parse inputs
    my $_resource_path = '/inboxes/available';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'email_address'}) {
        $query_params->{'emailAddress'} = $self->{api_client}->to_query_value($args{'email_address'});
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('EmailAvailableResult', $response);
    return $_response_object;
}

#
# list_inbox_rulesets
#
# List inbox rulesets
# 
# @param string $inbox_id  (required)
# @param int $page Optional page index in inbox ruleset list pagination (optional, default to 0)
# @param int $size Optional page size in inbox ruleset list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param string $search_filter Optional search filter (optional)
# @param DateTime $since Optional filter by created after given date time (optional)
# @param DateTime $before Optional filter by created before given date time (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'page' => {
        data_type => 'int',
        description => 'Optional page index in inbox ruleset list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in inbox ruleset list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'search_filter' => {
        data_type => 'string',
        description => 'Optional search filter',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Optional filter by created after given date time',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Optional filter by created before given date time',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'list_inbox_rulesets' } = { 
        summary => 'List inbox rulesets',
        params => $params,
        returns => 'PageRulesetDto',
        };
}
# @return PageRulesetDto
#
sub list_inbox_rulesets {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling list_inbox_rulesets");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/rulesets';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'search_filter'}) {
        $query_params->{'searchFilter'} = $self->{api_client}->to_query_value($args{'search_filter'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageRulesetDto', $response);
    return $_response_object;
}

#
# list_inbox_tracking_pixels
#
# List inbox tracking pixels
# 
# @param string $inbox_id  (required)
# @param int $page Optional page index in inbox tracking pixel list pagination (optional, default to 0)
# @param int $size Optional page size in inbox tracking pixel list pagination (optional, default to 20)
# @param string $sort Optional createdAt sort direction ASC or DESC (optional, default to 'ASC')
# @param string $search_filter Optional search filter (optional)
# @param DateTime $since Optional filter by created after given date time (optional)
# @param DateTime $before Optional filter by created before given date time (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'page' => {
        data_type => 'int',
        description => 'Optional page index in inbox tracking pixel list pagination',
        required => '0',
    },
    'size' => {
        data_type => 'int',
        description => 'Optional page size in inbox tracking pixel list pagination',
        required => '0',
    },
    'sort' => {
        data_type => 'string',
        description => 'Optional createdAt sort direction ASC or DESC',
        required => '0',
    },
    'search_filter' => {
        data_type => 'string',
        description => 'Optional search filter',
        required => '0',
    },
    'since' => {
        data_type => 'DateTime',
        description => 'Optional filter by created after given date time',
        required => '0',
    },
    'before' => {
        data_type => 'DateTime',
        description => 'Optional filter by created before given date time',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'list_inbox_tracking_pixels' } = { 
        summary => 'List inbox tracking pixels',
        params => $params,
        returns => 'PageTrackingPixelProjection',
        };
}
# @return PageTrackingPixelProjection
#
sub list_inbox_tracking_pixels {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling list_inbox_tracking_pixels");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/tracking-pixels';

    my $_method = 'GET';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # query params
    if ( exists $args{'page'}) {
        $query_params->{'page'} = $self->{api_client}->to_query_value($args{'page'});
    }

    # query params
    if ( exists $args{'size'}) {
        $query_params->{'size'} = $self->{api_client}->to_query_value($args{'size'});
    }

    # query params
    if ( exists $args{'sort'}) {
        $query_params->{'sort'} = $self->{api_client}->to_query_value($args{'sort'});
    }

    # query params
    if ( exists $args{'search_filter'}) {
        $query_params->{'searchFilter'} = $self->{api_client}->to_query_value($args{'search_filter'});
    }

    # query params
    if ( exists $args{'since'}) {
        $query_params->{'since'} = $self->{api_client}->to_query_value($args{'since'});
    }

    # query params
    if ( exists $args{'before'}) {
        $query_params->{'before'} = $self->{api_client}->to_query_value($args{'before'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageTrackingPixelProjection', $response);
    return $_response_object;
}

#
# search_inboxes
#
# Search all inboxes and return matching inboxes
# 
# @param SearchInboxesOptions $search_inboxes_options  (required)
{
    my $params = {
    'search_inboxes_options' => {
        data_type => 'SearchInboxesOptions',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'search_inboxes' } = { 
        summary => 'Search all inboxes and return matching inboxes',
        params => $params,
        returns => 'PageInboxProjection',
        };
}
# @return PageInboxProjection
#
sub search_inboxes {
    my ($self, %args) = @_;

    # verify the required parameter 'search_inboxes_options' is set
    unless (exists $args{'search_inboxes_options'}) {
      croak("Missing the required parameter 'search_inboxes_options' when calling search_inboxes");
    }

    # parse inputs
    my $_resource_path = '/inboxes/search';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    my $_body_data;
    # body params
    if ( exists $args{'search_inboxes_options'}) {
        $_body_data = $args{'search_inboxes_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('PageInboxProjection', $response);
    return $_response_object;
}

#
# send_email
#
# Send Email
# 
# @param string $inbox_id ID of the inbox you want to send the email from (required)
# @param SendEmailOptions $send_email_options  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to send the email from',
        required => '1',
    },
    'send_email_options' => {
        data_type => 'SendEmailOptions',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'send_email' } = { 
        summary => 'Send Email',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub send_email {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling send_email");
    }

    # verify the required parameter 'send_email_options' is set
    unless (exists $args{'send_email_options'}) {
      croak("Missing the required parameter 'send_email_options' when calling send_email");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # body params
    if ( exists $args{'send_email_options'}) {
        $_body_data = $args{'send_email_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# send_email_and_confirm
#
# Send email and return sent confirmation
# 
# @param string $inbox_id ID of the inbox you want to send the email from (required)
# @param SendEmailOptions $send_email_options  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to send the email from',
        required => '1',
    },
    'send_email_options' => {
        data_type => 'SendEmailOptions',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'send_email_and_confirm' } = { 
        summary => 'Send email and return sent confirmation',
        params => $params,
        returns => 'SentEmailDto',
        };
}
# @return SentEmailDto
#
sub send_email_and_confirm {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling send_email_and_confirm");
    }

    # verify the required parameter 'send_email_options' is set
    unless (exists $args{'send_email_options'}) {
      croak("Missing the required parameter 'send_email_options' when calling send_email_and_confirm");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/confirm';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # body params
    if ( exists $args{'send_email_options'}) {
        $_body_data = $args{'send_email_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('SentEmailDto', $response);
    return $_response_object;
}

#
# send_email_with_queue
#
# Send email with queue
# 
# @param string $inbox_id ID of the inbox you want to send the email from (required)
# @param boolean $validate_before_enqueue Validate before adding to queue (required)
# @param SendEmailOptions $send_email_options  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to send the email from',
        required => '1',
    },
    'validate_before_enqueue' => {
        data_type => 'boolean',
        description => 'Validate before adding to queue',
        required => '1',
    },
    'send_email_options' => {
        data_type => 'SendEmailOptions',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'send_email_with_queue' } = { 
        summary => 'Send email with queue',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub send_email_with_queue {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling send_email_with_queue");
    }

    # verify the required parameter 'validate_before_enqueue' is set
    unless (exists $args{'validate_before_enqueue'}) {
      croak("Missing the required parameter 'validate_before_enqueue' when calling send_email_with_queue");
    }

    # verify the required parameter 'send_email_options' is set
    unless (exists $args{'send_email_options'}) {
      croak("Missing the required parameter 'send_email_options' when calling send_email_with_queue");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/with-queue';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # query params
    if ( exists $args{'validate_before_enqueue'}) {
        $query_params->{'validateBeforeEnqueue'} = $self->{api_client}->to_query_value($args{'validate_before_enqueue'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # body params
    if ( exists $args{'send_email_options'}) {
        $_body_data = $args{'send_email_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# send_smtp_envelope
#
# Send email using an SMTP mail envelope and message body and return sent confirmation
# 
# @param string $inbox_id ID of the inbox you want to send the email from (required)
# @param SendSMTPEnvelopeOptions $send_smtp_envelope_options  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to send the email from',
        required => '1',
    },
    'send_smtp_envelope_options' => {
        data_type => 'SendSMTPEnvelopeOptions',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'send_smtp_envelope' } = { 
        summary => 'Send email using an SMTP mail envelope and message body and return sent confirmation',
        params => $params,
        returns => 'SentEmailDto',
        };
}
# @return SentEmailDto
#
sub send_smtp_envelope {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling send_smtp_envelope");
    }

    # verify the required parameter 'send_smtp_envelope_options' is set
    unless (exists $args{'send_smtp_envelope_options'}) {
      croak("Missing the required parameter 'send_smtp_envelope_options' when calling send_smtp_envelope");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/smtp-envelope';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # body params
    if ( exists $args{'send_smtp_envelope_options'}) {
        $_body_data = $args{'send_smtp_envelope_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('SentEmailDto', $response);
    return $_response_object;
}

#
# send_test_email
#
# Send a test email to inbox
# 
# @param string $inbox_id  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'send_test_email' } = { 
        summary => 'Send a test email to inbox',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub send_test_email {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling send_test_email");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/send-test-email';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type();

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# send_with_schedule
#
# Send email with with delay or schedule
# 
# @param string $inbox_id ID of the inbox you want to send the email from (required)
# @param SendEmailOptions $send_email_options  (required)
# @param DateTime $send_at_timestamp Sending timestamp (optional)
# @param int $send_at_now_plus_seconds Send after n seconds (optional)
# @param boolean $validate_before_enqueue Validate before adding to queue (optional)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of the inbox you want to send the email from',
        required => '1',
    },
    'send_email_options' => {
        data_type => 'SendEmailOptions',
        description => '',
        required => '1',
    },
    'send_at_timestamp' => {
        data_type => 'DateTime',
        description => 'Sending timestamp',
        required => '0',
    },
    'send_at_now_plus_seconds' => {
        data_type => 'int',
        description => 'Send after n seconds',
        required => '0',
    },
    'validate_before_enqueue' => {
        data_type => 'boolean',
        description => 'Validate before adding to queue',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'send_with_schedule' } = { 
        summary => 'Send email with with delay or schedule',
        params => $params,
        returns => 'ScheduledJobDto',
        };
}
# @return ScheduledJobDto
#
sub send_with_schedule {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling send_with_schedule");
    }

    # verify the required parameter 'send_email_options' is set
    unless (exists $args{'send_email_options'}) {
      croak("Missing the required parameter 'send_email_options' when calling send_with_schedule");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/with-schedule';

    my $_method = 'POST';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # query params
    if ( exists $args{'send_at_timestamp'}) {
        $query_params->{'sendAtTimestamp'} = $self->{api_client}->to_query_value($args{'send_at_timestamp'});
    }

    # query params
    if ( exists $args{'send_at_now_plus_seconds'}) {
        $query_params->{'sendAtNowPlusSeconds'} = $self->{api_client}->to_query_value($args{'send_at_now_plus_seconds'});
    }

    # query params
    if ( exists $args{'validate_before_enqueue'}) {
        $query_params->{'validateBeforeEnqueue'} = $self->{api_client}->to_query_value($args{'validate_before_enqueue'});
    }

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # body params
    if ( exists $args{'send_email_options'}) {
        $_body_data = $args{'send_email_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('ScheduledJobDto', $response);
    return $_response_object;
}

#
# set_inbox_favourited
#
# Set inbox favourited state
# 
# @param string $inbox_id ID of inbox to set favourite state (required)
# @param SetInboxFavouritedOptions $set_inbox_favourited_options  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => 'ID of inbox to set favourite state',
        required => '1',
    },
    'set_inbox_favourited_options' => {
        data_type => 'SetInboxFavouritedOptions',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'set_inbox_favourited' } = { 
        summary => 'Set inbox favourited state',
        params => $params,
        returns => 'InboxDto',
        };
}
# @return InboxDto
#
sub set_inbox_favourited {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling set_inbox_favourited");
    }

    # verify the required parameter 'set_inbox_favourited_options' is set
    unless (exists $args{'set_inbox_favourited_options'}) {
      croak("Missing the required parameter 'set_inbox_favourited_options' when calling set_inbox_favourited");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}/favourite';

    my $_method = 'PUT';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # body params
    if ( exists $args{'set_inbox_favourited_options'}) {
        $_body_data = $args{'set_inbox_favourited_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('InboxDto', $response);
    return $_response_object;
}

#
# update_imap_access
#
# 
# 
# @param UpdateImapAccessOptions $update_imap_access_options  (required)
# @param string $inbox_id Inbox ID (optional)
{
    my $params = {
    'update_imap_access_options' => {
        data_type => 'UpdateImapAccessOptions',
        description => '',
        required => '1',
    },
    'inbox_id' => {
        data_type => 'string',
        description => 'Inbox ID',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'update_imap_access' } = { 
        summary => '',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub update_imap_access {
    my ($self, %args) = @_;

    # verify the required parameter 'update_imap_access_options' is set
    unless (exists $args{'update_imap_access_options'}) {
      croak("Missing the required parameter 'update_imap_access_options' when calling update_imap_access");
    }

    # parse inputs
    my $_resource_path = '/inboxes/imap-access';

    my $_method = 'PATCH';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # query params
    if ( exists $args{'inbox_id'}) {
        $query_params->{'inboxId'} = $self->{api_client}->to_query_value($args{'inbox_id'});
    }

    my $_body_data;
    # body params
    if ( exists $args{'update_imap_access_options'}) {
        $_body_data = $args{'update_imap_access_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

#
# update_inbox
#
# Update Inbox. Change name and description. Email address is not editable.
# 
# @param string $inbox_id  (required)
# @param UpdateInboxOptions $update_inbox_options  (required)
{
    my $params = {
    'inbox_id' => {
        data_type => 'string',
        description => '',
        required => '1',
    },
    'update_inbox_options' => {
        data_type => 'UpdateInboxOptions',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'update_inbox' } = { 
        summary => 'Update Inbox. Change name and description. Email address is not editable.',
        params => $params,
        returns => 'InboxDto',
        };
}
# @return InboxDto
#
sub update_inbox {
    my ($self, %args) = @_;

    # verify the required parameter 'inbox_id' is set
    unless (exists $args{'inbox_id'}) {
      croak("Missing the required parameter 'inbox_id' when calling update_inbox");
    }

    # verify the required parameter 'update_inbox_options' is set
    unless (exists $args{'update_inbox_options'}) {
      croak("Missing the required parameter 'update_inbox_options' when calling update_inbox");
    }

    # parse inputs
    my $_resource_path = '/inboxes/{inboxId}';

    my $_method = 'PATCH';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept('*/*');
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # path params
    if ( exists $args{'inbox_id'}) {
        my $_base_variable = "{" . "inboxId" . "}";
        my $_base_value = $self->{api_client}->to_path_value($args{'inbox_id'});
        $_resource_path =~ s/$_base_variable/$_base_value/g;
    }

    my $_body_data;
    # body params
    if ( exists $args{'update_inbox_options'}) {
        $_body_data = $args{'update_inbox_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    my $response = $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    if (!$response) {
        return;
    }
    my $_response_object = $self->{api_client}->deserialize('InboxDto', $response);
    return $_response_object;
}

#
# update_smtp_access
#
# 
# 
# @param UpdateSmtpAccessOptions $update_smtp_access_options  (required)
# @param string $inbox_id Inbox ID (optional)
{
    my $params = {
    'update_smtp_access_options' => {
        data_type => 'UpdateSmtpAccessOptions',
        description => '',
        required => '1',
    },
    'inbox_id' => {
        data_type => 'string',
        description => 'Inbox ID',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'update_smtp_access' } = { 
        summary => '',
        params => $params,
        returns => undef,
        };
}
# @return void
#
sub update_smtp_access {
    my ($self, %args) = @_;

    # verify the required parameter 'update_smtp_access_options' is set
    unless (exists $args{'update_smtp_access_options'}) {
      croak("Missing the required parameter 'update_smtp_access_options' when calling update_smtp_access");
    }

    # parse inputs
    my $_resource_path = '/inboxes/smtp-access';

    my $_method = 'PATCH';
    my $query_params = {};
    my $header_params = {};
    my $form_params = {};

    # 'Accept' and 'Content-Type' header
    my $_header_accept = $self->{api_client}->select_header_accept();
    if ($_header_accept) {
        $header_params->{'Accept'} = $_header_accept;
    }
    $header_params->{'Content-Type'} = $self->{api_client}->select_header_content_type('application/json');

    # query params
    if ( exists $args{'inbox_id'}) {
        $query_params->{'inboxId'} = $self->{api_client}->to_query_value($args{'inbox_id'});
    }

    my $_body_data;
    # body params
    if ( exists $args{'update_smtp_access_options'}) {
        $_body_data = $args{'update_smtp_access_options'};
    }

    # authentication setting, if any
    my $auth_settings = [qw(API_KEY )];

    # make the API Call
    $self->{api_client}->call_api($_resource_path, $_method,
                                           $query_params, $form_params,
                                           $header_params, $_body_data, $auth_settings);
    return;
}

1;
