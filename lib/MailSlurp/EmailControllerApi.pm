...
# @param boolean $decode  (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'get_email_summary' } = { 
        summary => 'Get email data including headers but not body. Expects email to exist by ID. For emails that may not have arrived yet use the WaitForController.',
        params => $params,
        returns => 'EmailPreview',
        };
}
...
# @param string $thread_id  (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'get_email_thread' } = { 
        summary => 'Return email thread information. Use items endpoints to get messages for thread.',
        params => $params,
...
# 
# @param string $html_selector Optional inbox filter (optional)
