...
sub get_missed_email {
    ...
    my $_response_object = $self->{api_client}->deserialize('MissedEmailDto', $response);
    return $_response_object;
}
...
# 
{
    ...
    __PACKAGE__->method_documentation->{ 'restore_missed_emails' } = { 
...
# @return void
#
sub restore_missed_emails {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/missed-emails/restore';
...
sub wait_for_nth_missed_email {
    ...
    unless (exists $args{'index'}) {
      croak("Missing the required parameter 'index' when calling wait_for_nth_missed_email");
    }
