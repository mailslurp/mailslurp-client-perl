...
> PageSmsProjection get_all_sms_messages(phone_number => $phone_number, page => $page, size => $size, sort => $sort, since => $since, before => $before, search => $search, favourite => $favourite)
...
eval { 
    my $result = $api_instance->get_all_sms_messages(phone_number => $phone_number, page => $page, size => $size, sort => $sort, since => $since, before => $before, search => $search, favourite => $favourite);
    print Dumper($result);
};
if ($@) {
