...
[**get_sms_codes**](SmsControllerApi#get_sms_codes) | **POST** /sms/{smsId}/codes | Extract verification codes from an SMS
...
[**mark_all_sms_as_read**](SmsControllerApi#mark_all_sms_as_read) | **PATCH** /sms/read | Mark all SMS messages as read or unread
[**mark_sms_as_read**](SmsControllerApi#mark_sms_as_read) | **PATCH** /sms/{smsId}/read | Mark an SMS as read or unread
...
# **get_sms_codes**
> ExtractCodesResult get_sms_codes(sms_id => $sms_id, extract_codes_options => $extract_codes_options)
...
my $sms_id = null; # string | ID of SMS to extract codes from
my $extract_codes_options = MailSlurp::Object::ExtractCodesOptions->new(); # ExtractCodesOptions | 

eval { 
    my $result = $api_instance->get_sms_codes(sms_id => $sms_id, extract_codes_options => $extract_codes_options);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling SmsControllerApi->get_sms_codes: $@\n";
}
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **sms_id** | [**string**]()| ID of SMS to extract codes from | 
...
# **mark_all_sms_as_read**
> mark_all_sms_as_read(read => $read, phone_number_id => $phone_number_id)
...
my $read = true; # boolean | 
my $phone_number_id = null; # string | 

eval { 
    $api_instance->mark_all_sms_as_read(read => $read, phone_number_id => $phone_number_id);
};
if ($@) {
    warn "Exception when calling SmsControllerApi->mark_all_sms_as_read: $@\n";
}
```
...
# **mark_sms_as_read**
> SmsDto mark_sms_as_read(sms_id => $sms_id, read => $read)
...
eval { 
    my $result = $api_instance->mark_sms_as_read(sms_id => $sms_id, read => $read);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling SmsControllerApi->mark_sms_as_read: $@\n";
}
```
