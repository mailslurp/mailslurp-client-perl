...
[**get_available_domain_regions**](DomainControllerApi#get_available_domain_regions) | **GET** /domains/available-domain-regions | Get all usable domains with account region status
...
# **get_available_domain_regions**
> DomainRegionGroupsDto get_available_domain_regions(inbox_type => $inbox_type)
...
eval { 
    my $result = $api_instance->get_available_domain_regions(inbox_type => $inbox_type);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling DomainControllerApi->get_available_domain_regions: $@\n";
}
```
