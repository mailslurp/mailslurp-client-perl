...
# **get_alias_emails**
> PageEmailProjection get_alias_emails(alias_id => $alias_id, before => $before, page => $page, since => $since, size => $size, sort => $sort)
...
eval { 
    my $result = $api_instance->get_alias_emails(alias_id => $alias_id, before => $before, page => $page, since => $since, size => $size, sort => $sort);
    print Dumper($result);
};
if ($@) {
...
# **get_alias_threads**
> PageThreadProjection get_alias_threads(alias_id => $alias_id, before => $before, page => $page, since => $since, size => $size, sort => $sort)
...
eval { 
    my $result = $api_instance->get_alias_threads(alias_id => $alias_id, before => $before, page => $page, since => $since, size => $size, sort => $sort);
    print Dumper($result);
};
if ($@) {
...
eval { 
    my $result = $api_instance->get_aliases(before => $before, page => $page, since => $since, size => $size, sort => $sort);
    print Dumper($result);
};
if ($@) {
