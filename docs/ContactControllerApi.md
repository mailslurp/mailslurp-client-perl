...
# **get_all_contacts**
> PageContactProjection get_all_contacts(before => $before, page => $page, since => $since, size => $size, sort => $sort)
...
eval { 
    my $result = $api_instance->get_all_contacts(before => $before, page => $page, since => $since, size => $size, sort => $sort);
    print Dumper($result);
};
if ($@) {
