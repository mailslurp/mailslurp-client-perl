...
> PageInboxForwarderEvents get_all_inbox_forwarder_events(page => $page, size => $size, sort => $sort)
...
eval { 
    my $result = $api_instance->get_all_inbox_forwarder_events(page => $page, size => $size, sort => $sort);
    print Dumper($result);
};
if ($@) {
