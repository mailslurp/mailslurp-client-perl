...
> TotpDeviceCodeDto get_totp_device_code(id => $id, at => $at)
...
my $id = null; # string | 
my $at = DateTime->from_epoch(epoch => str2time('null')); # DateTime | 

eval { 
    my $result = $api_instance->get_totp_device_code(id => $id, at => $at);
    print Dumper($result);
};
if ($@) {
