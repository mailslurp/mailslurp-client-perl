...
> ARRAY[EmailPreview] wait_for_matching_emails(match_options => $match_options, count => $count, delay => $delay, inbox_id => $inbox_id, since => $since, sort => $sort, timeout => $timeout, unread_only => $unread_only)
...
eval { 
    my $result = $api_instance->wait_for_matching_emails(match_options => $match_options, count => $count, delay => $delay, inbox_id => $inbox_id, since => $since, sort => $sort, timeout => $timeout, unread_only => $unread_only);
    print Dumper($result);
};
if ($@) {
