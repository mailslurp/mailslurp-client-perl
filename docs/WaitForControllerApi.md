...
my $wait_for_conditions = MailSlurp::Object::WaitForConditions->new(); # WaitForConditions | Conditions to wait for
...
eval { 
    my $result = $api_instance->wait_for_nth_email(inbox_id => $inbox_id, index => $index, timeout => $timeout, unread_only => $unread_only);
    print Dumper($result);
};
if ($@) {
