...
> MissedEmail wait_for_nth_missed_email(inbox_id => $inbox_id, timeout => $timeout, index => $index)
...
eval { 
    my $result = $api_instance->wait_for_nth_missed_email(inbox_id => $inbox_id, timeout => $timeout, index => $index);
    print Dumper($result);
};
if ($@) {
