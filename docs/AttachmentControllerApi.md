...
> ARRAY[string] upload_attachment_bytes(inline_object1 => $inline_object1, content_type => $content_type, filename => $filename)
...
eval { 
    my $result = $api_instance->upload_attachment_bytes(inline_object1 => $inline_object1, content_type => $content_type, filename => $filename);
    print Dumper($result);
};
if ($@) {
