...
[**extract_attachment_text**](AttachmentControllerApi#extract_attachment_text) | **POST** /attachments/{attachmentId}/text | Extract text from an attachment
...
> ExtractAttachmentTextResult extract_attachment_text(attachment_id => $attachment_id, extract_attachment_text_options => $extract_attachment_text_options)
...
my $attachment_id = "attachment_id_example"; # string | ID of attachment
my $extract_attachment_text_options = MailSlurp::Object::ExtractAttachmentTextOptions->new(); # ExtractAttachmentTextOptions | 

eval { 
    my $result = $api_instance->extract_attachment_text(attachment_id => $attachment_id, extract_attachment_text_options => $extract_attachment_text_options);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling AttachmentControllerApi->extract_attachment_text: $@\n";
}
```
...
------------- | ------------- | ------------- | -------------
 **attachment_id** | **string**| ID of attachment | 
 **extract_attachment_text_options** | [**ExtractAttachmentTextOptions**](ExtractAttachmentTextOptions)|  | [optional] 
...
# **get_attachments**
> PageAttachmentEntity get_attachments(page => $page, size => $size, sort => $sort, file_name_filter => $file_name_filter, since => $since, before => $before, inbox_id => $inbox_id, email_id => $email_id, sent_email_id => $sent_email_id, include => $include)
...
eval { 
    my $result = $api_instance->get_attachments(page => $page, size => $size, sort => $sort, file_name_filter => $file_name_filter, since => $since, before => $before, inbox_id => $inbox_id, email_id => $email_id, sent_email_id => $sent_email_id, include => $include);
    print Dumper($result);
};
if ($@) {
...
> ARRAY[string] upload_multipart_form(content_id => $content_id, content_type => $content_type, filename => $filename, content_type_header => $content_type_header, x_filename => $x_filename, x_filename_raw => $x_filename_raw, x_filesize => $x_filesize, inline_object1 => $inline_object1)
...
my $inline_object1 = MailSlurp::Object::InlineObject1->new(); # InlineObject1 | 

eval { 
    my $result = $api_instance->upload_multipart_form(content_id => $content_id, content_type => $content_type, filename => $filename, content_type_header => $content_type_header, x_filename => $x_filename, x_filename_raw => $x_filename_raw, x_filesize => $x_filesize, inline_object1 => $inline_object1);
    print Dumper($result);
};
if ($@) {
 ...
 **inline_object1** | [**InlineObject1**](InlineObject1)|  | [optional] 
