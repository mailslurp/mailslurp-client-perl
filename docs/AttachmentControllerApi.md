...
# **get_attachments**
> PageAttachmentEntity get_attachments(before => $before, file_name_filter => $file_name_filter, page => $page, since => $since, size => $size, sort => $sort)
...
eval { 
    my $result = $api_instance->get_attachments(before => $before, file_name_filter => $file_name_filter, page => $page, since => $since, size => $size, sort => $sort);
    print Dumper($result);
};
if ($@) {
...
> ARRAY[string] upload_attachment_bytes(content_type => $content_type, filename => $filename, byte_array => $byte_array)
...
eval { 
    my $result = $api_instance->upload_attachment_bytes(content_type => $content_type, filename => $filename, byte_array => $byte_array);
    print Dumper($result);
};
if ($@) {
