...
# **get_expired_inboxes**
> PageExpiredInboxRecordProjection get_expired_inboxes(page => $page, size => $size, sort => $sort, since => $since, before => $before, inbox_id => $inbox_id)
...
my $inbox_id = null; # string | Optionally filter by inbox ID

eval { 
    my $result = $api_instance->get_expired_inboxes(page => $page, size => $size, sort => $sort, since => $since, before => $before, inbox_id => $inbox_id);
    print Dumper($result);
};
if ($@) {
 ...
 **inbox_id** | [**string**]()| Optionally filter by inbox ID | [optional] 
