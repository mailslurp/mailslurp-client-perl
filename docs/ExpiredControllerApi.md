...
# **get_expired_inboxes**
> PageExpiredInboxRecordProjection get_expired_inboxes(before => $before, page => $page, since => $since, size => $size, sort => $sort)
...
eval { 
    my $result = $api_instance->get_expired_inboxes(before => $before, page => $page, since => $since, size => $size, sort => $sort);
    print Dumper($result);
};
if ($@) {
