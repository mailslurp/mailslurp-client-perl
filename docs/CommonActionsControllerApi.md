...
my $favourite = null; # boolean | favourite
my $inbox_type = "inbox_type_example"; # string | inboxType
my $name = "name_example"; # string | name
my $tags = [("null")]; # ARRAY[string] | tags
my $use_domain_pool = null; # boolean | useDomainPool

eval { 
    my $result = $api_instance->create_new_email_address(allow_team_access => $allow_team_access, description => $description, email_address => $email_address, expires_at => $expires_at, expires_in => $expires_in, favourite => $favourite, inbox_type => $inbox_type, name => $name, tags => $tags, use_domain_pool => $use_domain_pool);
    print Dumper($result);
};
if ($@) {
...
eval { 
    my $result = $api_instance->create_new_email_address1(allow_team_access => $allow_team_access, description => $description, email_address => $email_address, expires_at => $expires_at, expires_in => $expires_in, favourite => $favourite, inbox_type => $inbox_type, name => $name, tags => $tags, use_domain_pool => $use_domain_pool);
    print Dumper($result);
};
if ($@) {
