...
eval { 
    $api_instance->get_bounced_email(id => $id);
};
if ($@) {
...
eval { 
    $api_instance->get_bounced_email1(id => $id);
};
if ($@) {
    warn "Exception when calling BounceControllerApi->get_bounced_email1: $@\n";
}
```
...
# **get_bounced_emails**
> get_bounced_emails(page => $page, size => $size, sort => $sort)
...
eval { 
    $api_instance->get_bounced_emails(page => $page, size => $size, sort => $sort);
};
if ($@) {
...
> get_bounced_recipients(page => $page, size => $size, sort => $sort)
...
eval { 
    $api_instance->get_bounced_recipients(page => $page, size => $size, sort => $sort);
};
if ($@) {
