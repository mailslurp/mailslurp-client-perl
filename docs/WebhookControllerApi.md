...
eval { 
    my $result = $api_instance->get_all_webhook_results(before => $before, page => $page, search_filter => $search_filter, since => $since, size => $size, sort => $sort, unseen_only => $unseen_only);
    print Dumper($result);
};
if ($@) {
...
# **get_webhook_results**
> PageWebhookResult get_webhook_results(webhook_id => $webhook_id, before => $before, page => $page, search_filter => $search_filter, since => $since, size => $size, sort => $sort, unseen_only => $unseen_only)
...
eval { 
    my $result = $api_instance->get_webhook_results(webhook_id => $webhook_id, before => $before, page => $page, search_filter => $search_filter, since => $since, size => $size, sort => $sort, unseen_only => $unseen_only);
    print Dumper($result);
};
if ($@) {
...
> UnseenErrorCountDto get_webhook_results_unseen_error_count(inbox_id => $inbox_id)
...
eval { 
    my $result = $api_instance->get_webhook_results_unseen_error_count(inbox_id => $inbox_id);
    print Dumper($result);
};
if ($@) {
