...
# **get_all_webhooks**
> PageWebhookProjection get_all_webhooks(page => $page, size => $size, sort => $sort, search_filter => $search_filter, since => $since, inbox_id => $inbox_id, ai_transformer_id => $ai_transformer_id, phone_id => $phone_id, before => $before, health => $health, event_type => $event_type, url => $url)
...
eval { 
    my $result = $api_instance->get_all_webhooks(page => $page, size => $size, sort => $sort, search_filter => $search_filter, since => $since, inbox_id => $inbox_id, ai_transformer_id => $ai_transformer_id, phone_id => $phone_id, before => $before, health => $health, event_type => $event_type, url => $url);
    print Dumper($result);
};
if ($@) {
